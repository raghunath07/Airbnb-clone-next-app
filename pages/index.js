        ...
        <section className="pt-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            {exploreData?.map(item => {
              return (<SmallCard image= {item.img} location={item.location} distance={item.distance} key={item.img}/>)
            })}
          </div>
        </section>
        <section>
          ...
          <div className="flex space-x-3 overflow-scroll scrollbar-hide p-3 ml-3">
            {cardData?.map(item => {
                return (<MediumCard image= {item.img} title={item.title} key={item.img}/>)
              })}
          </div>
        </section>
